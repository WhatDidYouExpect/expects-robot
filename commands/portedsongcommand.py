...
class MusicCommands(commands.Cog):
    ...
    async def song(self, ctx, song_name: discord.Option(str, "The name of the song you want to look for")):
        logger.info(f"User {ctx.author.name} (ID: {ctx.author.id}) ran the command: '{ctx.command.qualified_name}'")
        if ctx.author.id in banned_users:
            embed = create_ban_embed(ctx, ctx.author.id)  re
            await ctx.respond(embed=embed)
            return
        #### get access token ####
        client_credentials = f"{self.spotify_id}:{self.spotify_secret}"
